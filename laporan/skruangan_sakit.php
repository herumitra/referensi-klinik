...
?>
 ...
 <?php 
    $query_diagnosa = "Select * from tbl_diagnosa where no_daftar='$no_daftar'";
                ...
                $data_nosa = mysqli_fetch_array($sql_diag);
$istirahat = $data_nosa['istirahat'];
$pekerjaan = $data_nosa['pekerjaan'];
             ?>


 <?php 
    $query_narkoba = "Select * from tbl_suratsakit where no_daftar='$no_daftar'";
...
/*    $pdf->Cell(20,6,$data['nama'],1,0);
 ...
 $pdf->SetTextColor(0,0,0);
        ...
        $pdf->Cell(0,20,' SURAT KETERANGAN SAKIT ',0,1,'C');
        ...
        $pdf->Cell(0,45,'Perlu istirahat karena sakit selama : '.$istirahat.' ',0,1,'L');
        $pdf->Cell(15);
        ...
        $pdf->Cell(5,-17,'',0,1,'L');
        $pdf->Cell(15);
        $pdf->MultiCell(0,8,'Terhitung mulai tanggal : '.tgl_indo($datasu['tgl_istirahat']).' sampai dengan tanggal : '.tgl_indo($datasu['tgl_selesai']).' ');
         $pdf->Cell(15);
        $pdf->MultiCell(0,10,'Demikian Surat Keterangan Sakit dibuat, agar yang bersangkutan harap maklum  ');
         $pdf->Cell(125);
        ...
        $pdf->Cell(0,20,'' ,0,1,'L');
        $pdf->Cell(125);
        $pdf->Cell(0,-20,''.$datasu['nm_dokter'],0,1,'L');
