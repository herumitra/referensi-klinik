 ...
 */
    ...
    } else if(@$_GET['page']=='nota_pengajuan') {
        ob_start();
        include(dirname(__FILE__).'/isi/nota_pengajuan.php');
        $content = ob_get_clean();
        $format = new HTML2PDF('P', 'B5', 'en');
        $nama_file = "nota_pengajuan".$no_faktur.".pdf";



    } else if(@$_GET['page']=='nota_stokawal') {
        ob_start();
        include(dirname(__FILE__).'/isi/nota_stokawal2.php');
        $content = ob_get_clean();
        $format = new HTML2PDF('P', 'B5', 'en');
        $nama_file = "nota_stokawal".$no_faktur.".pdf";


    } else if(@$_GET['page']=='nota_pembayaran') {
        ob_start();
        include(dirname(__FILE__).'/isi/nota_pembayaran.php');
/*        $content = ob_get_clean();
        $format = new HTML2PDF('P', 'A8', 'en');
        $nama_file = "nota_pembayaran".$no_daf.".pdf";*/
