...
$hari_ini = date("d-m-Y");
   ...
   $query_daft = mysqli_query($mysqli,"SELECT * from tbl_daftarpasienranap   WHERE no_daftar ='$no_daftar' ");
   ...
   /* $query_tampil = "SELECT * FROM tbl_penjualan INNER JOIN tbl_pegawai ON tbl_penjualan.id_peg = tbl_pegawai.id_peg ORDER BY tbl_penjualan.tgl_penjualan DESC, tbl_penjualan.no_penjualan DESC";*/
    // fungsi query untuk menampilkan data dari tabel obat masuk
       ...
       $query = mysqli_query($mysqli,"SELECT tbl_pengobatandetail.*, tbl_pengobatandetail.kd_obat, tbl_pengobatan.no_daftar,  tbl_apotek.kd_obat, tbl_apotek.nm_obat, tbl_apotek.hrg_obat FROM tbl_pengobatandetail
                  LEFT JOIN tbl_pengobatan ON tbl_pengobatandetail.no_pengobatan=tbl_pengobatan.no_pengobatan
                  LEFT JOIN tbl_apotek ON tbl_pengobatandetail.kd_obat=tbl_apotek.kd_obat
...
$nama       = $data['nama_pas'];
    ...
    $subtotal_obat   = $data['jml_jual']* $data['hrg_jual'];

    $totalBayar = $totalBayar + $subtotal_obat;
            ...
            echo "  <tr>
                        ...
                        <td style='padding-right:10px;' width='10' height='3' align='right' valign='middle'>$data[hrg_jual]</td>
                            <td style='padding-right:10px;' width='25' height='3' align='right' valign='middle'>$subtotal_obat</td>
                    </tr>"
...
require_once "../config/database.php";
   ...
   $no    = 1;
          ...
          $query = mysqli_query($mysqli,"SELECT tbl_racikandetail.*, tbl_racikandetail.kd_racik, tbl_racikan.no_daftar, tbl_racikan.nama_pas, tbl_nama_racikan.kd_racik, tbl_nama_racikan.nama_racikan, tbl_nama_racikan.total_penjualan FROM tbl_racikandetail
...
require_once "../config/database.php";
   ...
   $no    = 1;
       ...
       $query_dp = mysqli_query($mysqli,"SELECT * FROM tbl_daftarpasienranap 
...
<?php
    ...
    // jika data tidak ada
        ...
        // tampilkan data
                while($data_rawat = mysqli_fetch_array($query_dp)) {
...
$total_hari = $data_rawat['tarif']*($diff->d+1);
            ...
            // menampilkan isi tabel dari database ke tabel di aplikasi
            echo "  <tr>
                          <td style='padding-left:5px;' width='55' height='3' valign='middle'>Tarif Kamar</td>

                        <td width='10' height='3' align='center' valign='middle'>$hari</td>
                        <td style='padding-right:10px;' width='10' height='3' align='right' valign='middle'>$tarif</td>
                            <td style='padding-right:10px;' width='25' height='3' align='right' valign='middle'>$total_hari</td>
                    </tr>"
...
$query_lab = mysqli_query($mysqli,"SELECT tbl_labordetail.*, tbl_labordetail.kd_labor, tbl_labor.no_daftar,tbl_labor.nm_dokter, data_labor.kd_labor, data_labor.nama_labor, data_labor.harga_labor FROM tbl_labordetail
                  LEFT JOIN tbl_labor ON tbl_labordetail.no_labor=tbl_labor.no_labor
                  LEFT JOIN data_labor ON tbl_labordetail.kd_labor=data_labor.kd_labor
                WHERE no_daftar ='$no_daftar' ");
$counlab  = mysqli_num_rows($query_lab);
?>
            <table width="50%" border="0" cellpadding="0" cellspacing="0">
                <thead style="background:#e8ecee">
                    <tr class="tr-title">
...
$totallabor = 0; 
    ...
    // jika data tidak ada
        ...
        // tampilkan data
        while ($datalab = mysqli_fetch_assoc($query_lab)) {
$dokter       = $datalab['nm_dokter'];

                        $subtotal   = $datalab['hrg_labor'];
    $totallabor = $totallabor + $subtotal;
            // menampilkan isi tabel dari database ke tabel di aplikasi
            echo "  <tr>

                        <td style='padding-left:5px;' width='76' height='3' valign='middle'>$datalab[nama_labor]</td>
                            <td style='padding-right:10px;' width='50' height='3' align='right' valign='middle'>$datalab[hrg_labor]</td>

                        <td style='padding-left:5px;' width='76' height='3' valign='middle'></td>
                            <td style='padding-right:10px;' width='50' height='3' align='right' valign='middle'></td></tr>
                    "
...
?>
  ...
  <tr >
    ...
    <td colspan='2' style='padding-right:10px;' width='50' height='3' align='right' ><?php echo ($totalBayar+$totalRacik+$totalTindakan+$total_hari+$totallabor); ?></td>
</tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan='2' style='padding-right:10px;' width='50' height='3' align='right' ><?php echo($totalBayar+$totalRacik+$totalTindakan+$total_hari+$admin); ?></td>
</tr>
<tr>
...
<?php
$filename="Laporan Billing Pasien.pdf"; //ubah untuk menentukan nama file pdf yang dihasilkan nantinya
//==========================================================================================================
