...
if (isset($_GET['tgl_awal'])) {
   ...
   /* $query_tampil = "SELECT * FROM tbl_penjualan INNER JOIN tbl_pegawai ON tbl_penjualan.id_peg = tbl_pegawai.id_peg ORDER BY tbl_penjualan.tgl_penjualan DESC, tbl_penjualan.no_penjualan DESC";*/
    ...
    $query = mysqli_query($mysqli, "SELECT * FROM tbl_daftarpasien_igd INNER JOIN tbl_pegawai ON tbl_daftarpasien_igd.id_peg = tbl_pegawai.id_peg  WHERE tbl_daftarpasien_igd.tgl_daftar BETWEEN '$tgl_awal' AND '$tgl_akhir' AND asuransi_pas ='$asuransi'  
                                    ") 
...
<html xmlns="http://www.w3.org/1999/xhtml"> <!-- Bagian halaman HTML yang akan konvert -->
    <head>
        ...
        <title>LAPORAN DATA PASIEN IGD</title>
        <link rel="stylesheet" type="text/css" href="../asset/laporan.css" />
    </head>
    <body>
        <div id="title">
            LAPORAN DATA PASIEN IGD 
        </div>
    <?php  
...
<?php
    ...
    if($count == 0) {
 $no    = 0;
        $query2 = mysqli_query($mysqli, "SELECT * FROM tbl_daftarpasien_igd INNER JOIN tbl_pegawai ON tbl_daftarpasien_igd.id_peg = tbl_pegawai.id_peg  WHERE tbl_daftarpasien_igd.tgl_daftar BETWEEN '$tgl_awal' AND '$tgl_akhir' 
                                    ") 
                                    or die('Ada kesalahan pada query tampil Transaksi : '.mysqli_error($mysqli));
...
<?php
$filename="LAPORAN DATA PASIEN IGD.pdf"; //ubah untuk menentukan nama file pdf yang dihasilkan nantinya
//==========================================================================================================
