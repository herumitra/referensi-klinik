...
<div class="page-content">
	<div class="row">
		<div class="col-6"><h4>Data Pengajuan Pembelian Obat</h4></div>
      ...
      <a href="?page=laporpengajuan" class="btn btn-sm btn-pink">
        Buat Laporan Pembelian
			...
			<a href="?page=entry_datapengajuan">
				<button class="btn btn-sm btn-info">Pengajuan Pembelian Obat</button>
			</a>
		</div>
	</div>
    <style>
    ...
    <div class="kotak-data-tab">
    	<ul class="nav nav-pills" id="pills-tab" role="tablist">
          <?php 
            $query_lunas = "SELECT * FROM tbl_pengajuan_pembelian INNER JOIN tbl_pegawai ON tbl_pengajuan_pembelian.id_peg = tbl_pegawai.id_peg INNER JOIN tbl_supplier ON tbl_pengajuan_pembelian.no_supplier = tbl_supplier.no_supp WHERE tbl_pengajuan_pembelian.status_byr = 'Disetujui' ORDER BY tbl_supplier.nama_supp ASC";
            $sql_lunas = mysqli_query($conn, $query_lunas) or die ($conn->error);
            $jml_lunas = mysqli_num_rows($sql_lunas);

            $query_utang = "SELECT * FROM tbl_pengajuan_pembelian INNER JOIN tbl_pegawai ON tbl_pengajuan_pembelian.id_peg = tbl_pegawai.id_peg INNER JOIN tbl_supplier ON tbl_pengajuan_pembelian.no_supplier = tbl_supplier.no_supp WHERE tbl_pengajuan_pembelian.status_byr = 'Pengajuan' ORDER BY tbl_pengajuan_pembelian.tgl_pembelian ASC";
                         ...
                         ?>
                            <tr>
                                ...
                                <td class="td-opsi">
                                    ...
                                    <a href="laporan/?page=nota_pengajuan&no_faktur=<?php echo $dlunas['no_faktur']; ?>" target="_blank">
...
<td>
   ...
   <button class="btn-transition btn btn-outline-success btn-sm" title="Edit Obat Racik" id="tombol_obat_pembelian" name="tombol_obat_pembelian" data-id="<?php echo $dlunas['no_faktur']; ?>">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
        	<table class="tabel-profil">
        		<tr>
        			<th>No Faktur</th>
        			<td id="no_fakturdetail"></td>
        			<th>Tanggal</th>
        			<td id="tgl_pembeliandetail"></td>
        		</tr>
        		<tr>
        			<th>Supplier</th>
...
<script>
	$("button[name='tombol_detail']").click(function() {
		...
		$.ajax({
			type: "GET",
			url: "ajax/detail.php?page=pengajuan",
...
$("button[name='tombol_obat_pembelian']").click(function() {
    var id = $(this).data('id');
    window.location='?page=pengajuan_pembelian_obat&id='+id;
  });
    ...
    $("button[name='tombol_pelunasan']").click(function() {
        ...
        Swal.fire({
          ...
          if (lunas.value) {
            $.ajax({
              type: "POST",
              url: "ajax/detail.php?page=pelunasan_pengajuan",
              data: "no_faktur="+no_faktur,
...
/*                    window.open("laporan/?page=nota_pengajuan&no_faktur="+no_faktur);*/
                    window.location='?page=datapengajuan_beli';
                  }
                })
              }
            })  
