...
<div class="page-content">
	<div class="row">
		<div class="col-6"><h4>Form Entry Distribusi Request</h4></div>
		<div class="col-6 text-right">
			<a href="?page=datapengajuan">
				<button class="btn btn-sm btn-info">Data Pembelian</button>
			</a>

            <a href="?page=lapor_distribusi">
                <button class="btn btn-sm btn-warning">Laporan Distribusi Obat</button>
            </a>
		</div>
	</div>
	<div class="form-container">
		<div class="row" style="padding: 0 16px;">
			<div class="col-md-12 vertical-form">
				<h6><i class="fas fa-list-alt"></i> Lengkapi form ini untuk menambah data Distribusi Request</h6>
...
<!--                             <label for="no_faktur" class="">Nomor Faktur <small>(nomor pada kertas faktur)</small></label> -->
                ...
                <?php 
                    ...
                    $hari= substr($tgl_permintaan, 8, 2);
                    $bulan = substr($tgl_permintaan, 5, 2);
                    $tahun = substr($tgl_permintaan, 0, 4);
                    $tgl = $tahun.$bulan.$hari;
                    $carikode = mysqli_query($conn, "SELECT MAX(no_permintaan) FROM tbl_pengajuan WHERE tgl_permintaan = '$tgl_permintaan'") or die (mysql_error());
                    $datakode = mysqli_fetch_array($carikode);
                    if($datakode) {
                        $nilaikode = substr($datakode[0], 13);
                        $kode = (int) $nilaikode;
                        $kode = $kode + 1;
                        $no_permintaan = "DRE/".$tgl."/".str_pad($kode, 2, "0", STR_PAD_LEFT);
                    } else {
                        $no_permintaan = "DRE/".$tgl."/01";
                    }
                 ?>

                <div style="text-align: left;">
                    No Distribusi Request : <b><?php echo $no_permintaan; ?></b> <br>
                ...
                <form method="post" id="form_penjualan" autocomplete="off">
                    <div class="position-relative row form-group">
                        ...
                        <div class="col-sm-4">
                            <input name="no_permintaan" id="no_permintaan" placeholder="nomor permintaan" type="hidden" class="form-control form-control-sm" value="<?php echo $no_permintaan; ?>">
                        </div>
                        <!-- <label for="tanggal_pjl" class="col-sm-2 col-form-label">Periode Penjualan</label> -->
                        <div class="col-sm-4">
                            <input name="tgl_permintaan" id="tgl_permintaan" type="hidden" class="form-control form-control-sm" value="<?php echo date('Y-m-d'); ?>">
                        </div>
                    </div>
...
<!--                             <div class="input-group input-group-sm">
                        ...
                        <div class="position-relative form-group col-md-4">
                            <label for="nm_supplier" class="">Apotek</label>
                            <div class="input-group">
                                <input type="text" class="form-control form-control-sm" id="nm_depo" name="nm_depo" value ='Apotek'>
                                <input type="hidden" class="form-control form-control-sm" id="no_depo" name="no_depo">
                                <div class="input-group-append">
                        ...
                        <div class="position-relative form-group col-md-4">
                            <label for="to unit" class="">Reques To Unit</label>
                            <div class="input-group">

                      <select name="request_unit" id="request_unit" class="form-control form-control-sm">
                        <option value="Gudang Farmasi">Gudang Farmasi</option>
                        <option value="Depo IGD">Depo IGD</option>
                        <option value="Depo Rajal">Depo Rajal</option>
                        <option value="Depo Ranap">Depo Ranap</option>
...
<!--                         <div class="position-relative form-group col-md-4">
                            <label for="tgl_permintaan" class="">Tanggal Permintaan</label>
                            <div class="input-group date">
                                <input type="text" class="form-control form-control-sm datepicker" id="tgl_permintaan" name="tgl_permintaan">
                    ...
                    <div class="row tombol-kanan">
                        <div class="col-md-7 data-pembelian row">
                            ...
                            <div class="position-relative form-group col-md-4">
                                <label for="no_faktur" class="">Pengajuan</label>
                            ...
                            <div class="position-relative form-group col-md-4">
                                <label for="jth_tempo" class="">Tgl Pengajuan</label>
                                <input type="text" class="form-control form-control-sm datepicker_jthtempo" id="jth_tempo" name="jth_tempo"   placeholder="masukkan tgl jatuh tempo">
                            </div>
                        </div>
...
<div class="modal fade" id="modal_datasupplier" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Data Apotek</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
       ...
       <!--              <td><?php echo $data_supp['alm_supp']; ?></td> -->
                    <td class="td-opsi">
                        <button class="btn-transition btn btn-outline-dark btn-sm" title="pilih" id="tombol_pilihsupp" name="tombol_pilihsupp" data-dismiss="modal"
                            data-no_depo="<?php echo $data_supp['no_depo']; ?>"
                            data-nama_depo="<?php echo $data_supp['nama_depo']; ?>"
                        >pilih</button>
                    </td>
                </tr>
         <?php 
...
$(document).ready(function() {
    ...
    $("button[name='tombol_pilihsupp']").click(function() {
        var no_depo = $(this).data("no_depo");
        var nama_depo = $(this).data("nama_depo");
        $("#nm_depo").val(nama_depo);
        $("#no_depo").val(no_depo);
    });
...
/*        } else if(exp=="") {
    ...
    $("#simpan_pembelian").click(function () {
        ...
        var no_depo = $("#no_depo").val();
        var tgl_permintaan = $("#tgl_permintaan").val();
        var request_unit = $("#request_unit").val();
/*        var jth_tempo = $("#jth_tempo").val();*/

        if(no_permintaan=="") {
            document.getElementById("no_depo").focus();
            Swal.fire(
              'Data Belum Lengkap',
            ...
            document.getElementById("lihat_data_apotek").focus();
            Swal.fire(
              'Data Belum Lengkap',
              'maaf, tolong pilih Apotek',
              'warning'
            )
        } else if(tgl_permintaan=="") {
            document.getElementById("tgl_permintaan").focus();
            Swal.fire(
              'Data Belum Lengkap',
                        ...
                        success:function(data) {
                            Swal.fire({
                              ...
                              if (ok.value) {
                                ...
                                if(cr_byr=="Pengajuan") {
                                    window.open("laporan/?page=nota_pengajuan&no_faktur="+no_faktur);
                                }
                                location.reload();
                              }
                            })
