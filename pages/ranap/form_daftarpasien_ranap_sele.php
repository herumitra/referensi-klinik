...
$kama = mysqli_query($conn, "SELECT * FROM ruangan ORDER BY nama_kamar");
while($p=mysqli_fetch_array($kama)){
...
<?php
//mengambil nama-nama propinsi yang ada di database
$beda = mysqli_query($conn, "SELECT * FROM bed ORDER BY nama_bed");
while($p=mysqli_fetch_array($kama)){
echo "<option value=\"$p[nama_bed]\">$p[nama_bed]</option>\n";
}
?>
</select>
 </div>
 ...
 <div class="form-group row">
					...
					<label for="" class="col-sm-2 col-form-label" style="text-align: right;"> </label>
				    <div class="col-sm-4">
<!-- 				      <input type="text" class="form-control form-control-sm" name="deposit" id="deposit" placeholder="masukkan Deposit" > -->
					</div>
				  </div>
