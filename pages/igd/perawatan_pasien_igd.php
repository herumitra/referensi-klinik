...
<div class="page-content">
	<div class="row">
		<div class="col-6" id="judul"><h4>Data Rawat Pasien IGD </h4></div>
...
<!-- 					<th>Upload</th> -->
		...
		<?php 
			$query_tampil = "SELECT * FROM tbl_daftarpasien_igd where  tgl_daftar = CURDATE() AND status='rawat' "  ;
...
<td><button class="btn-transition btn btn-outline-primary btn-sm" title="Diagnosa" id="tombol_diagnosa_igd" name="tombol_diagnosa_igd" data-id="<?php echo $data['no_daftar']; ?>">
...
<?php } ?>
			...
			<div class="page-content">
	<div class="row">
		<div class="col-6" id="judul"><h4>Data Rawat Pasien IGD <?php echo $_SESSION['jenis_poli']; ?> </h4></div>
		<div class="col-6 text-right">
			<a href="?page=bankdata">
				<button class="btn btn-sm btn-info">Pendaftaran Pasien</button>
			</a>
...
<!-- 					<th>Upload</th> -->
		...
		<?php 
			$query_tampil = "SELECT * FROM tbl_daftarpasien_igd where proses = '".$_SESSION['jenis_poli']."'  AND tgl_daftar = CURDATE() AND status='rawat' ORDER BY no_antrian DESC"  ;
			$sql_tampil = mysqli_query($conn, $query_tampil) or die ($conn->error);
			while($data = mysqli_fetch_array($sql_tampil)) {
		 ?>
		 		<tr>
...
<script>
	...
	$("button[name='tombol_diagnosa_igd']").click(function() {
		var id = $(this).data('id');
		window.location='?page=diagnosa_igd&id='+id;
	});
	...
	$("button[name='tombol_obatpasien']").click(function() {
		var id = $(this).data('id');
		window.location='?page=entry_obatpasien_igd&id='+id;
	});

	$("button[name='tombol_obatracik']").click(function() {
		var id = $(this).data('id');
		window.location='?page=entry_obatracik_igd&id='+id;
	});

		$("button[name='tombol_tindakan']").click(function() {
		var id = $(this).data('id');
		window.location='?page=entry_tindakanpasien_igd&id='+id;
	});


			$("button[name='tombol_penunjang']").click(function() {
		var id = $(this).data('id');
		window.location='?page=entry_penunjang_igd&id='+id;
	});
