...
<div class="page-content">
	<div class="row">
		<div class="col-6" id="judul"><h4>Data Riwayat Pasien IGD</h4></div>
		<div class="col-6 text-right">
			<a href="?page=bankdata">
				<button class="btn btn-sm btn-info">Pendaftaran Pasien</button>
			</a>

			            <a href="?page=laporpasien_igd">
                <button class="btn btn-sm btn-warning">Laporan Pasien IGD</button>
...
<!--        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-primary">
	...
	<div class="table-container">
		<table id="example" class="table table-striped display tabel-data">
		    ...
		    <tbody>
		<?php 
			$query_tampil = "SELECT * FROM tbl_daftarpasien_igd  ORDER BY tgl_daftar asC"  ;
		 ...
		 ?>
		 		<tr>
		 			...
		 			<br><small>MR : <?php echo $data['nomor_rm']; ?></small>
		 			<br><small>Lahir : <?php echo tgl_indo($data['lhr_pas']); ?></small>
		 			<br><small><?php echo $data['jk_pas']; ?></small></td>
		 			<td width="20%"><?php echo $data['alm_pas']; ?>
		 			<br><small>HP: <?php echo $data['no_hp']; ?></small></td>
		 			<td><?php echo $data['tgl_daftar']; ?></td>
		 			<td><?php echo $data['diagnosa']; ?></td>


<td><div class="btn-group">
								<a class="btn-transition btn btn-outline-primary btn-sm" href="#"><i class="icon-wrench icon-white"></i> No Surat</a>
								<a class="btn btn-info dropdown-toggle" data-toggle="dropdown" href="#"><span class="caret"></span></a>
								<ul class="dropdown-menu">
									<li> 
                        <button class="btn-transition btn btn-outline-primary btn-sm" title="No Surat Sehat" id="tombol_suratsehat" name="tombol_suratsehat" data-id="<?php echo $data['no_daftar']; ?>">
	                            <i class="fas fa-plus-circle"></i> Surat Sehat
	                        </button> </li>


                        <button class="btn-transition btn btn-outline-primary btn-sm" title="No Surat Sehat" id="tombol_suratberobat" name="tombol_suratberobat" data-id="<?php echo $data['no_daftar']; ?>">
	                            <i class="fas fa-plus-circle"></i> Surat Berobat
	                        </button> </li>

									<li> 
                        <button class="btn-transition btn btn-outline-primary btn-sm" title="No Surat Sakit" id="tombol_suratsakit" name="tombol_suratsakit" data-id="<?php echo $data['no_daftar']; ?>">
	                            <i class="fas fa-plus-circle"></i> Surat Sakit
	                        </button> </li>

									<li> 
                        <button class="btn-transition btn btn-outline-primary btn-sm" title="No Surat Rujukan" id="tombol_suratrujukan" name="tombol_suratrujukan" data-id="<?php echo $data['no_daftar']; ?>">
	                            <i class="fas fa-plus-circle"></i> Surat Rujukan
	                        </button> </li>
...
<td><div class="btn-group">
								<a class="btn-transition btn btn-outline-primary btn-sm" href="#"><i class="icon-wrench icon-white"></i> Print</a>
								<a class="btn btn-info dropdown-toggle" data-toggle="dropdown" href="#"><span class="caret"></span></a>
								<ul class="dropdown-menu">
									<li> 
...
<a href="laporan/skruangan_sakit.php?no_daftar=<?php echo $data['no_daftar']; ?>" target="_blank">
                              <button class="btn-transition btn btn-outline-danger btn-sm" title="cetak Surat Sakit" id="tombol_cetak" name="tombol_cetak">
                                  <i class="fas fa-print"></i> Surat Sakit </a> </li>
									...
									<li> 
<a href="laporan/skruangan_berobat.php?no_daftar=<?php echo $data['no_daftar']; ?>" target="_blank">
                              <button class="btn-transition btn btn-outline-danger btn-sm" title="cetak Surat Berobat" id="tombol_cetak" name="tombol_cetak">
                                  <i class="fas fa-print"></i> Surat Berobat</a> </li>

									<li> 
<a href="laporan/skruangan_rujukan.php?no_daftar=<?php echo $data['no_daftar']; ?>" target="_blank">
                              <button class="btn-transition btn btn-outline-danger btn-sm" title="cetak Surat Berobat" id="tombol_cetak" name="tombol_cetak">
                                  <i class="fas fa-print"></i> Surat Rujukan</a> </li>
		 			...
		 			<td width="10%" class="td-opsi">
		 				...
		 				data-asuransi="<?php echo $data['asuransi_pas']; ?>">
	                        ...
	                        <button class="btn-transition btn btn-outline-primary btn-sm" title="Rekam Medis" id="tombol_rekammedis" name="tombol_rekammedis" data-id="<?php echo $data['nomor_rm']; ?>">
...
<script>
	...
	$("button[name='tombol_hapus']").click(function() {
		...
		if(id==id_session) {
			...
			Swal.fire({
	          ...
	          text: 'anda akan menghapus data '+nama+', semua data yang berkaitan dengan pasien ini akan ikut terhapus',
	          type: 'warning',
	          ...
	          if (hapus.value) {
	            $.ajax({
	              type: "POST",
	              url: "ajax/hapus.php?page=datamedis",
	              data: "id="+id,
	...
	$("button[name='tombol_rekammedis']").click(function() {
		var id = $(this).data('id');
		window.location='?page=rekammedis_igd&id='+id;
	});

	$("button[name='tombol_suratsehat']").click(function() {
		var id = $(this).data('id');
		window.location='?page=suratsehat&id='+id;
	});

	$("button[name='tombol_suratberobat']").click(function() {
		var id = $(this).data('id');
		window.location='?page=suratberobat&id='+id;
	});

	$("button[name='tombol_suratsakit']").click(function() {
		var id = $(this).data('id');
		window.location='?page=suratsakit&id='+id;
	});


	$("button[name='tombol_suratrujukan']").click(function() {
		var id = $(this).data('id');
		window.location='?page=suratrujukan&id='+id;
	});
