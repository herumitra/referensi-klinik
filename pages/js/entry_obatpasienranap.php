...
<div class="page-content">
    <div class="row">
        <div class="col-6"><h4>Form Pemberian Obat Ranap</h4></div>
                        ...
                        <div class="position-relative form-group col-md-3">
                            ...
                            <div class="input-group ">
                              <input type="text" class="form-control form-control-sm" id="norem" name="norem" placeholder="masukkan nama pasien" autofocus="" value="<?php echo $data['nomor_rm']; ?>" >
                                                            ...
                                                            <input type="hidden" class="form-control form-control-sm" id="status" name="status" placeholder="masukkan status" autofocus="" value="Tetap" >
                            </div>
                        </div>

                        <div class="position-relative form-group col-md-3">
                            <label for="nm_supplier" class="">Lokasi</label>
                            <div class="input-group">
                                <input type="text" class="form-control form-control-sm" id="nm_depo" name="nm_depo" value ='Apotek'> 
                                <input type="hidden" class="form-control form-control-sm" id="no_depo" name="no_depo" value ='3'>
                                <div class="input-group-append">
...
/*            $query_tampil = "SELECT * FROM tbl_dataobat INNER JOIN tbl_stokexpobat ON tbl_dataobat.kd_obat = tbl_stokexpobat.kd_obat WHERE tbl_stokexpobat.tgl_exp > date_add(CURDATE(), INTERVAL 10 DAY) AND tbl_stokexpobat.stok > 0 ORDER BY tbl_dataobat.nm_obat ASC";*/
 ...
 $dewan1 = $conn->prepare($query_pjlobat);
         $dewan1->execute();
              ...
              while ($row = $res1->fetch_assoc()) {
                ...
                <tr>
                 ...
                 <td>'.$row["jml_jual"].'</td>
                  <td>'.$row["hrg_jual"].'</td>
                  <td>'.$row["hrg_jual"]*$row["jml_jual"].'</td>
                </tr>
                ';
             }
            ?>
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Data Lokasi</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
            ...
            Swal.fire({
              ...
              if (simpan.value) {
                ...
                if(count_data > 0) {
                    ...
                    $.ajax({
                        url: "ajax/simpan_pengobatan_ranap.php",
                        method: "POST",
                        data: form_data,
