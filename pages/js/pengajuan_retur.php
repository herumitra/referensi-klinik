 ...
 <h5> Retur Obat  </h5>
        <table  id="editable_table" class="table table-striped display tabel-data">
            ...
            <?php 
                ...
                $nomor = 1;
                     $query_pjlobat = "SELECT tbl_dataobat.nm_obat, tbl_pengobatandetail.no_pengobatan,tbl_pengobatandetail.hrg_jual, tbl_pengobatandetail.jml_jual, tbl_pengobatandetail.sat_jual, tbl_pengobatandetail.subtotal, tbl_pengobatandetail.jml_retur, tbl_pengobatandetail.no FROM tbl_pengobatandetail INNER JOIN tbl_dataobat ON tbl_pengobatandetail.kd_obat = tbl_dataobat.kd_obat WHERE tbl_pengobatandetail.no_pengobatan = '$no_pengobatan'";
...
*/
        $kode=$row['no_pengobatan'];       
        $no=$row['no'];       
                echo '
                <tr>
                 <td>'.$row["no"].'</td>
                 <td>'.$row["no_pengobatan"].'</td>
                 <td>'.$row["nm_obat"].'</td>
                  <td>'.$row["jml_jual"].'</td>
                  <td>'.$row["hrg_jual"].'</td>
                  <td>'.$row["jml_retur"].'</td>
            ...
            ?>
                             ...
                             <tbody>
        <?php 
   $query_tampil = "SELECT tbl_dataobat.nm_obat, tbl_pengobatandetail.no_pengobatan,tbl_pengobatandetail.hrg_jual, tbl_pengobatandetail.jml_jual, tbl_pengobatandetail.sat_jual, tbl_pengobatandetail.subtotal FROM tbl_pengobatandetail INNER JOIN tbl_dataobat ON tbl_pengobatandetail.kd_obat = tbl_dataobat.kd_obat WHERE tbl_pengobatandetail.no_pengobatan = '$no_pengobatan'";

/*   "SELECT * from tbl_nama_racikandetail where kd_racik='$kd_racik'";           */
$sql_tampil = mysqli_query($conn, $query_tampil) or die ($conn->error);
            while($data = mysqli_fetch_array($sql_tampil)) {
                $kode =$data['no_pengobatan'];
/*                $nopen =$data['no_pengobatan'];*/
         ?>
           <?php } ?>
...
<br>
     <div class="row" style="padding: 0 16px;">
            <div class="col-md-12 vertical-form">
 <h5>  Konfirmasi Obat Masuk ke Depo  </h5>
        <table  id="editable_table" class="table table-striped display tabel-data">
            <thead>
                <tr>
                    <th>No</th>
            ...
            <?php 
                ...
                $nomor = 1;
$query_pjlo = "SELECT tbl_dataobat.nm_obat, tbl_pengobatan_retur.no_pengobatan,tbl_pengobatan_retur.hrg_jual, tbl_pengobatan_retur.jml_jual, tbl_pengobatan_retur.sat_jual, tbl_pengobatan_retur.subtotal , tbl_pengobatan_retur.jml_retur, tbl_pengobatan_retur.status FROM tbl_pengobatan_retur INNER JOIN tbl_dataobat ON tbl_pengobatan_retur.kd_obat = tbl_dataobat.kd_obat WHERE tbl_pengobatan_retur.no_pengobatan = '$no_pengobatan'";
        ...
        $kode=$datak['no_pengobatan'];       
                echo '
                <tr>
                 <td>'.$datak["no_pengobatan"].'</td>
                 <td>'.$datak["nm_obat"].'</td>
                  <td>'.$datak["jml_jual"].'</td>
                  <td>'.$datak["sat_jual"].'</td>
                  <td>'.$datak["subtotal"].'</td>
        <td>'.$datak["jml_retur"].'</td>
            ...
            ?>
                             ...
                             <tbody>
        <?php 
   $query_tampil =  "SELECT tbl_dataobat.nm_obat, tbl_pengobatan_retur.no_pengobatan,tbl_pengobatan_retur.hrg_jual, tbl_pengobatan_retur.jml_jual, tbl_pengobatan_retur.sat_jual, tbl_pengobatan_retur.subtotal, tbl_pengobatan_retur.kd_obat FROM tbl_pengobatan_retur INNER JOIN tbl_dataobat ON tbl_pengobatan_retur.kd_obat = tbl_dataobat.kd_obat WHERE tbl_pengobatan_retur.no_pengobatan = '$no_pengobatan'";
...
$("button[name='tombol_simpanan']").click(function() {
        ...
        $("#no_pengobatan").val(id);
        $("#no_pengobatan").val(no_pengobatan);
        $("#kd_obat").val(kd_obat);

        window.location='?page=simpanan_retur&id='+id;
    });
 ...
 <script>  
    $(document).ready(function(){  
      $('#editable_table').Tabledit({
        url:'pages/js/action_retur.php',
        columns:{
          identifier:[0, "no"],
          editable:[[3, 'jml_jual'], [4, 'hrg_jual'], [5, 'jml_retur']]
        },
        restoreButton:true,
